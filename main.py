...
class Bot(BaseBot):
    ...
    async def on_chat(self, user: User, message: str):
        try:
            ...
            if message.startswith("!all h")and user.username in co_mod: 
               roomUsers = (await self.highrise.get_room_users()).content
               for roomUser, _ in roomUsers:
            ...
            if message.lstrip().startswith(("!v","!g","!dj","!h","!r","!s")):
                response = await self.highrise.get_room_users()
                ...
                try:
                    ...
                    if message.startswith("!s")and user.username in co_mod: 
            ...
            if message.lower().startswith("!loop all") and user.username  in moderator:
              parts = message.split()
              if len(parts) < 2:
